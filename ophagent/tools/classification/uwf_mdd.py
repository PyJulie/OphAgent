"""UWF Multi-Disease Detection via UWF-MDD (reused model)."""
from __future__ import annotations
from typing import Any, Dict
from ophagent.tools.base import BaseTool, CondaToolMixin, ToolMetadata


class UWFMDDTool(CondaToolMixin, BaseTool):
    """
    Ultra-widefield fundus multi-disease detection.
    Runs in a dedicated Conda environment.
    ...
    def run(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if Path(script).resolve() == Path(__file__).resolve():
            raise RuntimeError("No standalone UWF-MDD conda entrypoint configured.")
    ...
    def fallback_run(self, inputs: Dict[str, Any], error: Exception) -> Dict[str, Any]:
        from ophagent.utils.fallback_inference import uwf_mdd_prediction
        return uwf_mdd_prediction(inputs["image_path"], error=error)
