...
# -*- coding: utf-8 -*-
"""
oct_all_in_one.py — 厚み/表面 .npy → en‑face → 乳頭中心 → 環状解析＋cpRNFLT（ワンストップ）

この版の主な追加:
  • --target に "disc2" を追加（3重円: 直径 2.8 / 3.46 / 4.4 mm のリング帯内外で面積平均）
  • disc2 では 各リング帯ごとに Total / 上半分(UH) / 下半分(LH) / TSNIT（T,S,N,I）/ 12h（30°×12）を出力
  • 図も 3 本の円＋TSNIT の 45°/135°/… 放射＋12h 放射をリング帯（2.8–4.4 mm）内のみに描画
  • cpRNFLT は 3.46 mm 径（固定）の 12分割（従来通り）。OS は左右反転して右眼基準に統一。

既存の disc / macula の動作・入出力は維持しています。
"""

from __future__ import annotations
import os, sys, argparse, subprocess, warnings, re
...
# ---- 既存モジュール ----------------------------------------------------------
import make_enface_bm as ef                     # en‑face 生成
from annulus_analysis import analyse_annulus    # 従来の disc 環状解析（完全トレース）
...
RADIUS_MM = 3.46        # ★ 3.46 mm に統一
VOXEL_X   = 17.6        # µm/px（円周サンプリングの x 方向スケール）
R_PX      = RADIUS_MM * 1_000 / VOXEL_X   # ≈ px

# ---- SURFS / LAYERS / VMAP（既存の定義を忠実に） ------------------------------
...
def _normalize_laterality(val: Any) -> Optional[str]:
    ...
    if s in {"od", "o.d.", "o d"}: return "OD"
    if s in {"os", "o.s.", "o s"}: return "OS"
    if s in {"r", "right", "rt", "右", "od/右", "right eye", "r-eye"}: return "OD"
    if s in {"l", "left", "lt", "左", "os/左", "left eye", "l-eye"}:     return "OS"
    if s.startswith("r-") or s.endswith("-r"): return "OD"
    if s.startswith("l-") or s.endswith("-l"): return "OS"
    if "right" in s: return "OD"
    if "left"  in s: return "OS"
    if s.endswith(" od") or s.endswith(" os"): return s[-2:].upper().strip()
    return None
...
def determine_od_os_from_fda(fda_path: Path) -> Tuple[Optional[str], Dict[str, Any]]:
    ...
    # Primary: capture_info_02.eye (0→OD, 1→OS)
    try:
        ...
        if ci2 is not None and ("eye" in ci2):
            ...
            if iv in (0, 1):
                ...
                dbg["hit_key"] = "capture_info_02.eye"; dbg["hit_value"] = val; dbg["method"] = "capture_info_02.eye"
                return lat, dbg
    except Exception:
        pass

    # Fallback: generic key search
    ...
    for keypath, value in _iter_kv(meta):
        if any(k in keypath.lower() for k in ["eye","lat","od","os","side"]):
            s = str(value);  s = s if len(s) <= 80 else s[:80]
...
def _order_six_by_depth(vol_surfs: np.ndarray):
    ...
    def score(order):
        ...
        gcc_med    = float(np.nanmedian(vol_surfs[idx["GCL_IPL"]] - vol_surfs[idx["ILM"]]))
        ok_rnfl   = (rnfl_med > 0)
        ok_retina = (retina_med >= max(gcc_med, rnfl_med + gcl_med) - 10.0)
...
def make_enface_for_ids(ids: List[str], oct_root: Path, enface_dir: Path,
                        seg_mode: str, ai_ckpt: Optional[Path], gpu: str) -> None:
...
def _macula_masks(shape, cx, cy, scan_range_mm):
    ...
    r = np.hypot(dx, dy)
    theta = (np.degrees(np.arctan2(dy, dx)) + 360.0) % 360.0  # 0°=Temporal(right), CCW
...
def analyse_macula(core_id: str, layer_name: str, thick_map: np.ndarray,
    ...
    # 1) ETDRS overlay (45°/135° 斜線、中心円は貫かない)
    fig1 = plt.figure(figsize=(6,6), dpi=150); ax1 = plt.gca()
    im = ax1.imshow(thick_map, cmap="viridis", vmin=vmin, vmax=vmax, origin="upper")
    r0, r1, r2 = masks["radii_px"]
    ax1.add_patch(plt.Circle((cx,cy), r0, fill=False, color="w", lw=1.5))
    for rr in [r1, r2]:
    ...
    for ang in (45, 135, 225, 315):
        ...
        L = 1024.0
        ax1.plot([cx + ux*r0, cx + ux*L], [cy - uy*r0, cy - uy*L], 'w--', lw=1.0)
        ax1.plot([cx - ux*r0, cx - ux*L], [cy + uy*r0, cy + uy*L], 'w--', lw=1.0)
    ...
    ax1.text(cx, cy-r1*0.65, "iSUP", color="w", ha="center", va="bottom", fontsize=8)
    ...
    row = {
        "core_id": core_id, "layer": layer_name,
        "global": f"{global_mean:.2f}", "upper": f"{upper_mean:.2f}", "lower": f"{lower_mean:.2f}",
    }
    for k in [f"G{i}{j}" for i in range(1,5) for j in range(1,5)]:
        row[k] = f"{grid_means[k]:.2f}"
    for k in ["C","iSUP","iNAS","iINF","iTEM","oSUP","oNAS","oINF","oTEM"]:
        row[k] = f"{etdrs_means[k]:.2f}"
    df = pd.DataFrame([row])
    return figs, df

# ---- disc2: 3 重円のリング帯解析 --------------------------------------------
def _theta_radius_mm(shape, cx, cy, scan_range_mm):
    H, W = shape
    yy, xx = np.indices((H, W))
    dx = xx - cx
    dy = cy - yy  # up positive
    r_px = np.hypot(dx, dy)
    mm_per_px = scan_range_mm / 512.0
    r_mm = r_px * mm_per_px
    theta = (np.degrees(np.arctan2(dy, dx)) + 360.0) % 360.0  # 0°=Temporal(right), CCW
    return yy, xx, r_mm, theta

def _sector_mask(base_mask, theta, lo, hi):
    if lo < hi:
        return base_mask & (theta >= lo) & (theta < hi)
    else:
        return base_mask & ((theta >= lo) | (theta < hi))

def _t_s_n_i_means(arr, base_mask, theta):
    tmask = _sector_mask(base_mask, theta, 315, 360) | _sector_mask(base_mask, theta, 0, 45)
    smask = _sector_mask(base_mask, theta, 45, 135)
    nmask = _sector_mask(base_mask, theta, 135, 225)
    imask = _sector_mask(base_mask, theta, 225, 315)
    def mean(mask): 
        v = arr[mask]
        return float('nan') if v.size==0 else float(np.nanmean(v))
    return mean(tmask), mean(smask), mean(nmask), mean(imask)

def _twelve_hour_means(arr, base_mask, theta):
    vals = {}
    for k in range(12):
        lo, hi = k*30, (k+1)*30
        m = _sector_mask(base_mask, theta, lo, hi)
        v = arr[m]
        vals[f"S{k+1}"] = float('nan') if v.size==0 else float(np.nanmean(v))
    return vals

def analyse_disc2(core_id: str, layer_name: str, thick_map: np.ndarray,
                  eye: str, scan_range_mm: float, cx: float, cy: float,
                  d_in: float=2.8, d_mid: float=3.46, d_out: float=4.4,
                  vmin: float=0.0, vmax: float=500.0):
    """
    3 本の同心円（直径 mm）→ 内側 [d_in..d_mid]、外側 [d_mid..d_out] のリング帯で集計。
    返り値: (figs(dict), df(pandas.DataFrame rows=2))
    """
    yy, xx, r_mm, theta = _theta_radius_mm(thick_map.shape, cx, cy, scan_range_mm)
    r0, r1, r2 = d_in/2.0, d_mid/2.0, d_out/2.0  # radii in mm

    inner = (r_mm >= r0) & (r_mm < r1)
    outer = (r_mm >= r1) & (r_mm < r2)
    upper = (yy < cy); lower = ~upper

    rows = []
    for region_name, base in (("inner", inner), ("outer", outer)):
        total = float(np.nanmean(thick_map[base])) if np.any(base) else float('nan')
        uh    = float(np.nanmean(thick_map[base & upper])) if np.any(base & upper) else float('nan')
        lh    = float(np.nanmean(thick_map[base & lower])) if np.any(base & lower) else float('nan')
        T,S,N,I = _t_s_n_i_means(thick_map, base, theta)
        h12 = _twelve_hour_means(thick_map, base, theta)
        row = {
            "core_id": core_id, "layer": layer_name, "Region": region_name,
            "Total": f"{total:.2f}", "UH": f"{uh:.2f}", "LH": f"{lh:.2f}",
            "T": f"{T:.2f}", "S": f"{S:.2f}", "N": f"{N:.2f}", "I": f"{I:.2f}",
        }
        for k in range(1,13):
            row[f"S{k}"] = f"{h12[f'S{k}']:.2f}"
        rows.append(row)

    # ---- 図（リング帯内のみ線を引く） ------------------------------------
    px_per_mm = 512.0 / scan_range_mm
    R0 = r0 * px_per_mm
    R1 = r1 * px_per_mm
    R2 = r2 * px_per_mm

    fig = plt.figure(figsize=(6,6), dpi=150); ax = plt.gca()
    im = ax.imshow(thick_map, cmap="viridis", vmin=vmin, vmax=vmax, origin="upper")
    for rr,ls,lw in ((R0,"--",1.0),(R1,"-",1.5),(R2,"--",1.0)):
        ax.add_patch(plt.Circle((cx,cy), rr, fill=False, color="w", lw=lw, ls=ls))
    # TSNIT boundaries 45°,135°,225°,315°（中心側は R0 から、外側は R2 まで）
    for ang in (45,135,225,315):
        rad = np.deg2rad(ang)
        ux, uy = np.cos(rad), -np.sin(rad)
        ax.plot([cx + ux*R0, cx + ux*R2], [cy - uy*R0, cy - uy*R2], "w--", lw=1.0, alpha=0.9)
    # 12h（30°刻み）
    for ang in range(0,360,30):
        rad = np.deg2rad(ang)
        ux, uy = np.cos(rad), -np.sin(rad)
        ax.plot([cx + ux*R0, cx + ux*R2], [cy - uy*R0, cy - uy*R2], "w:", lw=0.8, alpha=0.6)

    ax.text(cx+R1*0.85, cy, "T", color="w", ha="left", va="center", fontsize=10)
    ax.text(cx-R1*0.85, cy, "N", color="w", ha="right", va="center", fontsize=10)
    ax.text(cx, cy-R1*0.85, "S", color="w", ha="center", va="bottom", fontsize=10)
    ax.text(cx, cy+R1*0.85, "I", color="w", ha="center", va="top", fontsize=10)

    ax.set_title(f"{core_id} {layer_name} disc2 (2.8–3.46 / 3.46–4.4 mm) ({eye})")
    ax.set_xlim(0,512); ax.set_ylim(512,0); ax.axis("off")
    plt.colorbar(im, fraction=0.046, pad=0.04)
    figs = {"disc2_annuli": fig}
...
def compute_cprnflt12_row(pid: str, source_tag: str, eye: str, rnfl_xz: np.ndarray, cx: float, cz: float) -> Dict[str, str]:
    ...
    return {
        "PID": pid, "Source": source_tag, "Eye": eye,
...
# ---- 選択（1 core につき ai と topcon を両採用） ----------------------------
def select_per_core_multi(npy_paths: List[Path], npy_kind_arg: str) -> List[Tuple[str, Path, str, int, str]]:
    ...
    for cid, paths in groups.items():
        ...
        for p in paths:
            ...
            try:
                ...
                shape = vol.shape
                # 3D であればレイヤー数だけ見て採用可否を決める
                nlay = shape[0] if len(shape) == 3 else 0
            except Exception:
                nlay = 0
...
def parse_args():
    ...
    ap.add_argument("--outer", type=float, default=3.46, help="測定外径 [mm]（disc 用）")
    ap.add_argument("--inner", type=float, default=3.46, help="測定内径 [mm]（disc 用）")
    ap.add_argument("--scan_range_mm", type=float, default=6.0, help="解析面 512px の一辺 [mm]")

    ap.add_argument("--cp_append", action="store_true", help="cpRNFLT_12.csv を追記モードで出力（既定は新規作成/上書き）")

    # ★ disc2 用
    ap.add_argument("--d2_inner", type=float, default=2.8,  help="disc2: 内側リング下限 直径 [mm]")
    ap.add_argument("--d2_mid",   type=float, default=3.46, help="disc2: 内外の境界 直径 [mm]")
    ap.add_argument("--d2_outer", type=float, default=4.4,  help="disc2: 外側リング上限 直径 [mm]")

    ap.add_argument("--target", choices=["disc","macula","disc2"], default="disc",
                    help="解析対象: 視神経乳頭 (disc) / 黄斑 (macula) / disc2(3重円)")
...
def main():
    ...
    print("=== oct_all_in_one.py (disc/disc2/macula; OD/OS-aware) ===")
    print(f"[CFG] out_dir={OUT_DIR}  cp_append={args.cp_append}  seg_mode={args.seg_mode}  scan_range_mm={args.scan_range_mm}")

    all_paths = list_npys(THICK_DIR, args.pattern)
    if not all_paths:
        print(f"[ERROR] no npy found under {THICK_DIR} with '{args.pattern}'"); sys.exit(1)

    selected = select_per_core_multi(all_paths, args.npy_kind)
    if not selected:
        print("[ERROR] no usable npy after selection"); sys.exit(1)
    ids = sorted({cid for cid, *_ in selected})
    print(f"[INFO] selected cores: {len(ids)}  (total items={len(selected)})")

    # en‑face生成と中心推定（disc / disc2 のとき必要）
    if args.target in ("disc", "disc2"):
        ...
        # macula: center = image center (or CSV)
        if args.center_csv:
            id2center = load_centers_csv(Path(args.center_csv))
    ...
    rows_annulus: List[pd.DataFrame] = []
    ...
    for core_id, npy_path, npy_kind, nlay, src_tag in selected:
            ...
            if npy_kind == 'seg':
                if nlay >= 9:
                    for lname,(top,bot) in LAYERS_FROM_SURFS.items():
                        ...
                        figs, df = analyse_macula(core_id, lname, thick512, eye, args.scan_range_mm, cx=cx_eff, cy=cy, vmin=vmin, vmax=vmax)
                        df.insert(1, 'Eye', eye); df.insert(2, 'Source', src_tag)
                        for key, fig in figs.items():
                            fig.savefig(out_dir / f"{lname}_{key}.png", dpi=300)
                            plt.close(fig)
                ...
                for li, lname in enumerate(layer_names_thick[:vol.shape[0]]):
                    ...
                    for key, fig in figs.items():
                        ...
                        plt.close(fig)
                    rows_annulus.append(df)
            continue  # macula は cpRNFLT なし

        if args.target == 'disc2':
            # --- DISC2 MODE ---
            if npy_kind == "seg":
                if nlay >= 9:
                    for lname,(top,bot) in LAYERS_FROM_SURFS.items():
                        ...
                        thick512, cx_eff = flip_if_os_512(thick512, cx, eye)
                        vmin, vmax = VMAP.get(lname, (0, 500))
                        figs, df = analyse_disc2(core_id, lname, thick512, eye, args.scan_range_mm,
                                                 cx=cx_eff, cy=cy, d_in=args.d2_inner, d_mid=args.d2_mid, d_out=args.d2_outer,
                                                 vmin=vmin, vmax=vmax)
                        df.insert(1, 'Eye', eye); df.insert(2, 'Source', src_tag)
                        for key, fig in figs.items():
                            fig.savefig(out_dir / f"{lname}_{key}.png", dpi=300)
                    ...
                    idx_map, dbg = label_topcon6_surfaces(vol)
                    if not idx_map:
                        print(f"[WARN] {core_id}: cannot label 6 surfaces reliably → skip disc2")
                    else:
                        if args.gclplus_mode == 'gclipl':
                            L6 = {'RNFL':('ILM','RNFL_GCL'),'GCL+':('RNFL_GCL','IPL_INL'),
                                  'GCC':('ILM','GCL_IPL'),'Retina':('ILM','RPE_BM'),'Choroid':('RPE_BM','CHOROID_OUT')}
                    ...
                    print(f"[WARN] {core_id}: seg with {nlay} surfaces is unsupported (<6) → skip disc2")
                    rnfl_xz = None
            else:
                for li, lname in enumerate(layer_names_thick[:vol.shape[0]]):
                    layer_wz = vol[li]
        ...
        # ---- cpRNFLT（disc / disc2） -------------------------------------
        if (args.target in ('disc','disc2')) and (rnfl_xz is not None):
    ...
    if rows_annulus:
        ...
        elif args.target == 'disc2':
            ann_csv = Path(args.out_dir) / 'annulus_disc2.csv'
            final_df.to_csv(ann_csv, index=False)
            print('✔ annulus_disc2.csv saved:', ann_csv)
            print('[annulus_disc2.csv] columns:', list(final_df.columns))
        else:
    ...
    if (args.target in ('disc','disc2')) and cp_rows:
