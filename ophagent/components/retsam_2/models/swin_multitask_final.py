...
class CoordinateHead(nn.Module):
    ...
    def __init__(
        self,
        feature_size: int = 768,  # SwinTransformer最后一层的特征维度
        hidden_size: int = 512,
        num_coordinates: int = 2,  # 默认预测x,y坐标
        dropout: float = 0.1,
    ...
    def forward(self, x):
        ...
        Args:
            x: 特征图 [B, C, H, W]
        Returns:
            coordinates: 预测的坐标 [B, num_coordinates]，范围在[0,1]
        """
        batch_size = x.size(0)
        
        if self.use_attention:
            # 使用注意力机制
            # 重塑特征图: [B, C, H, W] -> [B, H*W, C]
            B, C, H, W = x.shape
...
class Swin_Multitask_Final(nn.Module):
    ...
    def __init__(
        ...
        enable_coordinate_prediction: bool = False,  # 是否启用坐标预测
        num_coordinates: int = 2,  # 要预测的坐标数量
        coordinate_hidden_size: int = 512,  # 坐标预测头的隐藏层大小
        coordinate_dropout: float = 0.1,  # 坐标预测头的dropout率
        use_attention_head: bool = False,  # 是否在坐标预测头中使用注意力机制
    ) -> None:
        """
        Args:
        ...
        if self.enable_coordinate_prediction:
            # 计算最后一层特征的维度
            # 根据实际的SwinViT架构，最终输出维度是 feature_size * (2 ** (len(depths) - 1)) * 2
            # 对于 feature_size=128, depths=(2,2,18,2): 128 * (2^3) * 2 = 128 * 8 * 2 = 2048
...
def create_swin_multitask_final_model(config_name="default"):
    """
    创建预定义配置的Swin_Multitask_Final模型
    
    Args:
        config_name: 配置名称，支持 "default", "large", "tiny", "coordinate"
    
    Returns:
        Swin_Multitask_Final模型实例
    """
    configs = {
        "default": {
            "in_channels": 3,
            "out_channels": (3, 3, 5, 4, 2, 11, 5, 2),  # 8个分割任务
            "patch_size": 4,
            "window_size": 10,
            "feature_size": 48,
            "depths": (2, 2, 18, 2),
...
if __name__ == "__main__":
    print("🔬 Swin_Multitask_Final 模型测试")
    print("=" * 50)
    ...
    for config_name in configs_to_test:
        ...
        try:
            ...
            print(f"  🎯 分割任务数: {model.tasks}")
            print(f"  📍 坐标预测: {'启用' if model.enable_coordinate_prediction else '禁用'}")
            
            if model.enable_coordinate_prediction:
                segmentation_logits, coordinates = outputs
                print(f"  📤 分割输出数量: {len(segmentation_logits)}")
                for i, logit in enumerate(segmentation_logits):
                    print(f"    任务{i}: {logit.shape}")
                print(f"  📤 坐标输出形状: {coordinates.shape}")
                print(f"  📍 坐标值范围: [{coordinates.min().item():.3f}, {coordinates.max().item():.3f}]")
                
                # 解析坐标
                coords = coordinates[0].cpu().numpy()
                num_points = len(coords) // 2
                print(f"  🔍 预测的{num_points}个关键点:")
                for i in range(num_points):
                    x_coord, y_coord = coords[i*2], coords[i*2+1]
                    print(f"    点{i+1}: ({x_coord:.3f}, {y_coord:.3f})")
            else:
    ...
    print(f"\n📋 使用说明:")
    print(f"1. 使用 create_swin_multitask_final_model() 创建预定义配置的模型")
    print(f"2. 通过 enable_coordinate_prediction=True 启用坐标预测功能")
    print(f"3. num_coordinates 必须是偶数（x,y坐标对）")
    print(f"4. 坐标输出范围在 [0,1]，需要乘以图像尺寸转换为像素坐标")
    print(f"5. 支持冻结/解冻编码器和坐标预测头")
    print(f"6. 可以同时进行多任务分割和坐标预测")
